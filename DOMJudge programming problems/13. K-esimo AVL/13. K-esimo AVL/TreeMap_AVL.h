...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int tami = 1, int nelemsSub = 1) : cv(e), iz(i), dr(d), altura(alt), tam_i(tami), nelemsSubarb(nelemsSub) {}
   };
...
public:
   ...
   clave_valor & kesimoMenor(int k) const{
	   Link p = busca_kesimoMenor(raiz, k);
	   if(p == nullptr) throw std::out_of_range("La clave no se puede consultar");
	   return p->cv;
   }
...
protected:
   ...
   Link busca_kesimoMenor(Link a, int k)const {
	   if (a == nullptr) return nullptr;
	   else if (a->tam_i > k) return busca_kesimoMenor(a->iz, k);
	   else if (a->tam_i < k) return busca_kesimoMenor(a->dr, k - a->tam_i);
	   else return a;
   }
   ...
   static int tami(Link a) {
	   if (a == nullptr) return 0;
	   else return a->tami;
   }

   static int nelemsSubarb(Link a) {
	   if (a == nullptr) return 0;
	   else return a->nelemsSubarb;
   }
   ...
   static void rotaDer(Link & k2) {
	  ...
	  int x = k1->tam_i-1;
	  int y = k1->nelemsSubarb - k1->tam_i;
	  int z = k2->nelemsSubarb - k2->tam_i;
	  
	  k2->tam_i = y + 1;
	  k1->nelemsSubarb = x + y + z + 2;
	  k2->nelemsSubarb = y + z + 1;
      k2 = k1;
   }
   ...
   static void rotaIzq(Link & k1) {
	  ...
	  k2->tam_i = x + y + 2;
	  k2->nelemsSubarb = x + y + z + 2;
	  k1->nelemsSubarb = x + y + 1;
      k1 = k2;
   ...
   static void reequilibraIzq(Link & a) {
      if (altura(a->dr) - altura(a->iz) > 1) {
         if (altura(a->dr->iz) > altura(a->dr->dr))
		  ...
		  a->tam_i = nelemsSubarb(a->iz)+1;
		  a->nelemsSubarb = a->tam_i + nelemsSubarb(a->dr);
	  }
   }
   ...
   static void subirMenor(Link & a, Link b, Link padre) {
      if (b->iz != nullptr){
         ...
         b->iz = a->iz;
		 b -> tam_i = a->tam_i;
		 b->nelemsSubarb = a->nelemsSubarb - 1;
         delete a;
         a = b;
		 a->altura = std::max(altura(a->iz), altura(a->dr)) + 1;
      }
